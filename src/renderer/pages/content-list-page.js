...
module.exports = class ContentList extends React.Component {
  render() {
    ...
    const contentElems = state.saved.contents.map((contentSummary, index) => this.renderContent(contentSummary, index));
  ...
  renderContentMetadata(contentSummary) {
    ...
    function renderContentStatus() {
      ...
      if (contentSummary.status === "paused") {
        ...
        // contentSummary.status is 'new' or something unexpected
        status = "";
      }
  ...
  renderContentButtons(contentSummary, index) {
    ...
    if (!contentSummary.error && ContentPlayer.isPlayableContentSummary(contentSummary)) {
      playButton = (
        <i key="play-button" title="Start streaming" className={"icon play"} onClick={dispatcher("playFile", infoHash, index)}>
          play_circle_outline
        </i>
      );
    }
    ...
    return (
      <div className="content-controls">
        {playButton}
        <i key="delete-button" className="icon delete" title="Remove content" onClick={dispatcher("confirmDeleteContent", infoHash, false)}>
          close
        </i>
      </div>
    );
  ...
  renderFileRow(contentSummary, file, index) {
    ...
    if (isPlayable) {
      ...
      handleClick = isDone ? dispatcher("openItem", infoHash, index) : e => e.stopPropagation(); // noop if file is not ready
    }
    ...
    return (
      <tr key={index} onClick={handleClick}>
        ...
        <td className="col-select" onClick={dispatcher("toggleContentFile", infoHash, index)}>
